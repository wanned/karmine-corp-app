...
export async function searchYoutube(query: string) {
  ...
  const ytInitialData = safeJsonParse(
    decodeURIComponent(
      html
  ...
  const itemSectionRenderer = itemSectionRendererSchema.safeParse(
    findItemSectionRenderer(ytInitialData.data)
  );
...
function parseVideo(data: any) {
  const videoWithContextRenderer = videoWithContextRendererSchema.safeParse(
    data.videoWithContextRenderer
  );

  const videoRenderer =
    videoWithContextRenderer.success ? videoWithContextRenderer : (
  ...
  let label = (
    'headline' in videoRenderer.data ?
      videoRenderer.data.headline
    : videoRenderer.data.title).runs[0].text.replace('\\\\', '\\');
  try {
