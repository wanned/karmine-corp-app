...
export const createEnvServiceImpl = ({ firebaseEnvShouldBeDefined = false } = {}) => {
  const FIREBASE_CLIENT_EMAIL = process.env.FIREBASE_CLIENT_EMAIL ?? '';
  const FIREBASE_PRIVATE_KEY = process.env.FIREBASE_PRIVATE_KEY ?? '';
  const FIREBASE_PROJECT_ID = process.env.FIREBASE_PROJECT_ID ?? '';

  if (firebaseEnvShouldBeDefined) {
    assertStringNotEmpty(FIREBASE_CLIENT_EMAIL, 'FIREBASE_CLIENT_EMAIL is not defined');
    assertStringNotEmpty(FIREBASE_PRIVATE_KEY, 'FIREBASE_PRIVATE_KEY is not defined');
    assertStringNotEmpty(FIREBASE_PROJECT_ID, 'FIREBASE_PROJECT_ID is not defined');
  }
    ...
    EnvService.of({
      getEnv: () =>
        Effect.succeed({
          ...
          VLR_GG_API_URL: 'https://www.vlr.gg',
...
function assertStringNotEmpty(value: string, message: string) {
  if (value === '') {
